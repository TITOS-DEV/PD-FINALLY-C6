...
/**
 * These tests hit the REAL Supabase Postgres instance from .env — there's
 * no local test database. Run with `pnpm test:e2e`. Each test either uses
 * the seeded accounts (see database/seeds/seed.sql) or creates its own
 * throwaway user with a random email, so runs don't collide with each other.
 */
describe("Auth flow (e2e)", () => {
  ...
  it("registers, logs in, and can call a protected route with the access token", async () => {
    ...
    expect(channels.body.channels).toEqual([]); // brand new user, no memberships yet
  });
  ...
  it("rotates the refresh token and rejects the old one on reuse", async () => {
    ...
    // Replaying the token that was already rotated must fail — this is the
    // "reuse detection" half of the rotation strategy.
