...
/**
 * e2e tests reuse the exact same `createApp()` the real server boots
 * (see src/server.ts) — no test-only Express setup that could drift from
 * production. supertest drives it directly over HTTP-in-memory, no open
 * port needed.
 */
...
/** a1111111... = "General" (admin, jhonatan, sofia). b2222222... = "Desarrollo Cohorte 6" (jhonatan, sofia only). */
